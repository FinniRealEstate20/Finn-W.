...
1. Käufer öffnet `/de/documents/wohnsitz-paderborn` → `external_link`-Branch zeigt einen Hero. State teilt sich in zwei: **(a)** wenn `pac:smart-fill:setup-done` im localStorage gesetzt ist → grüne **"Auto-Fill bereit"**-Karte mit großem CTA *"Jetzt ausfüllen lassen →"* und Aufklapp *"Wie genau läuft das ab?"*. **(b)** wenn Flag fehlt → blaue **"Auto-Fill für dieses Formular"**-Karte mit primärem CTA *"In 30 Sekunden einrichten →"* (führt zu `/bookmarklet-installer.html`) plus sekundärem *"Lieber manuell zum Portal"* und kleinem Escape-Hatch-Link *"Bereits eingerichtet? Hier markieren"* (setzt das Flag direkt). Die `DocumentItem`-Tiles in `/de/documents` tragen zusätzlich eine ⚡-Pille *"Auto-Fill bereit"* (grün) oder *"Auto-Fill nach Setup"* (blau).
2. Setup einmalig: Käufer öffnet `/bookmarklet-installer.html` → 3-Schritt-Wizard mit Browser-Detection (Mac/Chrome/Firefox-Tastenkombi auto-gewählt), animierter Drag-Target, Demo-Test-Lauf gegen `/test-fixtures/mein-digiport.html` und Bestätigungs-Button. Setup-Status-Flag `pac:smart-fill:setup-done` landet im `localStorage` (Origin der App). Der Installer lädt `public/bookmarklet.js` und encoded das JS als `javascript:`-URI mit `__PAC_API_BASE__` = `location.origin`.
...
- **Falsche Landingpage** (Recipe matched Host, aber kein Eingabe-Formular im DOM): Sidepanel rendert oben einen Hilfetext "Wir haben kein Eingabe-Formular auf dieser Seite gefunden — such nach 'Online anmelden', 'Tarif wählen' oder 'Umzug melden'". Verhindert User-Confusion auf Marketing-Startseiten. Die Westfalen-Weser-`externalUrl` zeigt jetzt direkt auf `/privatkunden/` statt der Marketing-Root, damit der User schon im richtigen Bereich startet. **TODO:** Tiefenlink bei Westfalen-Weser-Live-Site verifizieren — wir landen aktuell auf der Privatkunden-Übersicht, nicht direkt im Anmelde-Formular.
